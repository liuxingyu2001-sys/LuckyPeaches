...
本文对应 **2.2.9**。服务端和编译依赖均需更新为 `Liu-LuckyPeaches-2.2.9.jar`；不要把 LuckyPeaches 打包进调用插件，使用 `provided` 或 `compileOnly`，并声明 `softdepend: [LuckyPeaches]`。
...
| `setPlayerNotInBattle(Player)` | 清除标记，异步读取数据库并校准蟠桃 modifier，然后为存活玩家补满实际血量；null/离线忽略 |
...
屏蔽时当前血量超过新上限会被限制到上限，因此血条会变化；**2.2.9 起解除时，存活玩家立即恢复到实际最大血量**（含恢复后的蟠桃加成）；死亡/0 血玩家只恢复属性，不通过此 API 复活。解除支持退出、关服回调中 `isOnline()` 已为 false 的有效 Player 对象；调用方应在 `saveData()` 和背包同步插件取快照前解除。
...
PeachIntegrationAPI.setPeachBonusSuppressed(player, false); // 同步恢复属性，并为存活玩家补满血量
PeachIntegrationAPI.setPlayerNotInBattle(player);         // 在线时额外从数据库校准并补满
if (!player.isOnline()) {
...
死亡标记集合可并发访问；涉及 Player、属性或世界的调用应在主线程执行。数据库校准异步读取，应用前重新检查玩家在线状态、战斗标记与禁用世界，防止上一场的恢复任务干扰新决斗。数据库校准完成后，即使 modifier 数值没有变化，也会为存活玩家补满实际血量；若已开始新战斗、再次屏蔽、进入禁用世界或离线，则跳过旧回调。

新屏蔽 API 没有按调用插件区分所有权：同一玩家的生命周期应由一个管理者配对控制，多个插件不得各自解除同一个屏蔽标记。重载整个插件、不同步更新后端或外部插件直接改写蟠桃 modifier 不属于该保证范围。

## 管理命令

主命令 `/luckypeach`，保留别名 `/luckyp`。2.2.9 移除 `/lp` 别名以避免与 LuckPerms 冲突；需要重启重新注册命令，`/luckypeach reload` 不能卸载旧命令别名。旧消息文件中的内置帮助命令显示会自动转换为 `/luckypeach`，不覆盖自定义消息文件。
